...
describe('Attachments Module (e2e)', () => {
  ...
  it('POST /api/v1/attachments/upload should upload valid PDF file', async () => {
    const pdfBuffer = Buffer.from('%PDF-1.4 sample PDF binary payload');
  ...
  it('POST /api/v1/attachments/upload should reject oversized file (> 10MB) with 400', async () => {
    // 10.5 MB buffer
    ...
    await request(app.getHttpServer())
      ...
      .field('applicationId', applicationId)
      .field('type', 'RESUME')
      .field('label', 'Huge File')
      .attach('file', oversizedBuffer, 'huge.pdf')
      .expect(400);
  });

  it('GET /api/v1/attachments/:id/download should download file binary', async () => {
    const res = await request(app.getHttpServer())
    ...
    expect(res.body.toString()).toContain('%PDF-1.4 sample PDF binary payload');
  });
  ...
  it('GET /api/v1/attachments/:id/download after deletion should return 404', async () => {
    await request(app.getHttpServer())
      .get(`/api/v1/attachments/${uploadedAttachmentId}/download`)
      .set('Authorization', `Bearer ${authToken}`)
      .expect(404);
