...
import { EMAIL_PROVIDER_TOKEN, EmailProvider, SendEmailOptions, SendEmailResult, EmailAttachment } from '../interfaces/email-provider.interface';
...
export class EmailService {
  ...
  renderTemplate(templateName: string, context: Record<string, any>): string {
    ...
    if (!template) {
      ...
      if (!fs.existsSync(templatePath)) {
        throw new BadRequestException(`Email template '${cleanName}' does not exist at ${templatePath}`);
      }
  ...
  async send(options: SendEmailOptions) {
    this.logger.log(`Enqueuing raw email job for ${Array.isArray(options.to) ? options.to.join(', ') : options.to}`);
    return this.queueService.enqueue(QUEUE_NAMES.EMAIL, 'SEND_GENERIC_EMAIL', options);
  }
  ...
  async sendTemplate(options: SendTemplateOptions) {
    ...
    return this.queueService.enqueue(QUEUE_NAMES.EMAIL, 'SEND_TEMPLATE_EMAIL', emailPayload);
  }
  ...
  async sendBulk(
    ...
    this.logger.log(`Enqueuing bulk email jobs for ${recipients.length} recipients using template [${templateName}]`);
    const jobs: any[] = [];
  ...
  async sendDirect(options: SendEmailOptions, templateName = 'custom'): Promise<SendEmailResult> {
    const startTime = Date.now();
    const recipient = Array.isArray(options.to) ? options.to.join(', ') : options.to;

    try {
      const result: SendEmailResult = await this.emailProvider.sendEmail(options);
      ...
      if (this.metricsService) {
        ...
        this.metricsService.emailDurationSeconds.observe({ template: templateName }, durationMs / 1000);
      }
