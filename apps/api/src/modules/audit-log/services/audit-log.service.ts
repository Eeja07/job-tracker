...
import { AuditLogRepository, CreateAuditLogData, AuditLogSearchOptions } from '../../../repositories/audit-log/audit-log.repository';
...
export class AuditLogService {
  ...
  async recordEvent(data: CreateAuditLogData): Promise<AuditLog | void> {
    try {
      ...
      if (isHealthy) {
        await this.queueService.enqueue(QUEUE_NAMES.AUDIT, AuditJobName.RECORD_AUDIT_LOG, data);
        return;
      }
      this.logger.warn(`Audit queue is unavailable. Falling back to synchronous audit log database insertion.`);
      return await this.auditLogRepository.create(data);
    } catch (error: any) {
      this.logger.warn(`Failed to enqueue audit log job (${error.message}). Falling back to synchronous database insertion.`);
      return await this.auditLogRepository.create(data);
    }
  }
  ...
  async findByUser(userId: string, page = 1, limit = 20): Promise<{ logs: AuditLog[]; total: number }> {
  ...
  async searchLogs(options: AuditLogSearchOptions): Promise<{ logs: AuditLog[]; total: number }> {
