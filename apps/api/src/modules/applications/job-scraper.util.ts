        ...
        for (const match of jsonLdMatches) {
          try {
            ...
            if (jobPosting) {
              ...
              if (jobPosting.hiringOrganization?.name) companyName = String(jobPosting.hiringOrganization.name).trim();
              if (jobPosting.description) requirements = stripHtmlTags(String(jobPosting.description));
              ...
              if (jobPosting.baseSalary?.value) {
                ...
                salary = sal.value ? String(sal.value) : `${sal.minValue || ''} - ${sal.maxValue || ''} ${jobPosting.baseSalary.currency || ''}`;
              }
              break;
            }
          } catch {
