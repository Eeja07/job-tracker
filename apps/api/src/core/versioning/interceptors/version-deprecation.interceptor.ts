...
import { DEPRECATED_ENDPOINT_KEY, DeprecatedOptions } from '../decorators/deprecated-endpoint.decorator';
...
export class VersionDeprecationInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    const deprecatedMeta =
      this.reflector.get<DeprecatedOptions & { isDeprecated: boolean }>(DEPRECATED_ENDPOINT_KEY, handler) ||
      this.reflector.get<DeprecatedOptions & { isDeprecated: boolean }>(DEPRECATED_ENDPOINT_KEY, controllerClass);
    ...
    if (this.metricsService) {
      this.metricsService.apiVersionRequestsTotal.inc({ version, path, method });
    }
